...
type Server struct {
	*http.Server
	...
	// and the serverold to shut down.
...
//
// timeout is the duration to wait until killing active requests and stopping the serverold.
// If timeout is 0, the serverold never times out. It waits for all active requests to finish.
...
// down the serverold. The timeout value passed here will override the
// timeout given when constructing the serverold, as this is an explicit
// command to stop the serverold.
